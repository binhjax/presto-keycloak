...
// import com.sbm.keycloak.admin.client.demo.exceptions.Exception;
// import com.sbm.keycloak.admin.client.demo.exceptions.Exception;
// import com.sbm.keycloak.admin.client.demo.exceptions.Exception;
...
public class KeycloakAdminClient {
    ...
    // Get realm
    public RealmResource getRealmResource() throws Exception {
        if (keycloak == null) {
            throw new Exception("Realm Resource ...");
        }
    ...
    public UserRepresentation findUserByUUID(String uuid) {
        ...
        if (userResource.isPresent()) {
            try {
                Optional.of(userResource.get().toRepresentation()).orElseThrow(() -> new Exception(String.format("User with UUID : %s not found!", uuid).toString()));
    ...
    private boolean findClientAppByUUID(String clientId) {
        if (realmResource.clients().get(clientId) == null) {
            try {
                throw new Exception(String.format("Client with UUID : %s not found!", clientId).toString());
            } catch (Exception e) {
